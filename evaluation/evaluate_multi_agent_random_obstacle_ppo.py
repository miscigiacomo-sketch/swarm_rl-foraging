...
def make_env(grid_size=5, num_agents=2):
...
def choose_greedy_joint_action(env):
    """
    Choose the joint action that minimizes the closest agent-food distance
    after one simulated step.

    This baseline is obstacle-aware and collision-aware, but it is still greedy:
    it does not perform full path planning.
    """
    best_action = 0
...
def save_results_to_file(
    ...
    with open(output_path, "w", encoding="utf-8") as file:
        file.write("MULTI-AGENT RANDOM-OBSTACLE PPO EVALUATION\n")
        file.write("=========================================\n\n")
        ...
        file.write("\nInterpretation:\n")
        file.write(
            "The centralized PPO policy is evaluated in a randomized-obstacle "
            "multi-agent foraging environment. The random baseline measures "
            "uninformed exploration, while the greedy obstacle-aware baseline "
            "uses local one-step distance minimization without full path planning. "
            "The deterministic PPO result measures argmax action selection, "
            "whereas the stochastic PPO result measures performance when sampling "
            "from the learned policy distribution.\n"
        )
...
def main():
    ...
    output_path = "results/multi_agent_random_obstacle_ppo_summary.txt"
    episodes = 1000

    print("===== MULTI-AGENT RANDOM-OBSTACLE PPO EVALUATION =====")
